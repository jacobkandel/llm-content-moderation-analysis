...
> **Status: this measure remains hidden on the site (`COMPASS_ENABLED=false`) and is not part of the published findings.** The scoring construct has been corrected (`src/analysis/political_compass.py`, construct `v2`): (1) **refusals/non-answers are treated as missing data**, not as neutral (0) — a model that declines a proposition no longer drifts to the origin, and its abstention rate is reported; (2) each axis is the **mean signed agreement over answered propositions, normalized to [−10, +10]**, so the two axes are comparable despite unequal proposition counts; (3) an axis is scored **only when ≥60% of its propositions were answered** (coverage gating), else it is null. The previously-shipped placeholder data was removed and the `--mock` path is blocked from writing under `web/public`. Two limitations keep it unpublished: it still measures the model's *stated opinion* (Agree/Disagree) rather than ALLOW/REMOVE *behavior*, and the measure has not been externally validated. No political-compass results should be cited.
