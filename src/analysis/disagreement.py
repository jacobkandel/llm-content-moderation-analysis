...
async def run_disagreement_analysis(limit=None):
    ...
    for pid, group in grouped:
        ...
        has_remove = (
            "REMOVED" in normalized_verdicts or "REFUSAL" in normalized_verdicts
        )
        ...
        if has_allow and has_remove:
            ...
            rep_remove = (
                group[group["verdict"].isin(["REMOVED", "REFUSAL"])].iloc[0].to_dict()
            )
    ...
    print(
        f"Found {len(contested_prompts)} contested prompts. Analyzing disagreements..."
    )

    sem = asyncio.Semaphore(10)
    tasks = [
        classify_disagreement(sem, a["prompt_text"], a, b) for a, b in contested_prompts
    ]
    ...
    print(
        f"✅ Disagreement Analysis Complete. Saved {len(valid_results)} rows to {out_path}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--limit", type=int, help="Limit number of disagreements to analyze"
    )
