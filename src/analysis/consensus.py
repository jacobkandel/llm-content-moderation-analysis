...
# "Council of Judges"
JUDGES = ["google/gemini-2.0-flash-001", "deepseek/deepseek-chat", "openai/gpt-4o-mini"]
...
async def run_consensus_analysis(limit=None):
    ...
    if not os.path.exists(csv_path):
        print("❌ Could not find audit_log.csv.gz")
        return
    ...
    refusals = df[df["verdict"].isin(["REMOVED", "REFUSAL", "unsafe"])].to_dict(
        "records"
    )
    ...
    async def analyze_row_wrapper(row):
        try:
            ...
            tasks = [
                get_judge_verdict(client, sem, judge, prompt_content)
                for judge in JUDGES
            ]
            ...
            return {
                ...
                "agreement_score": (
                    max(vote_counts.values()) / len(votes) if votes else 0
                ),
            }
        except Exception as e:
            print(f"Row error: {e}")
