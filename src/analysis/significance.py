...
def calculate_significance():
    ...
    try:
        ...
        df["is_refusal"] = df["verdict"].apply(
            lambda x: 1 if x in refusal_keywords else 0
        )

        # Pivot: Index=Prompt, Columns=Model, Values=is_refusal
        # We need prompts that are present for BOTH models to compare
        pivot = df.pivot_table(
            index="prompt_text", columns="model", values="is_refusal", aggfunc="max"
        )
        ...
        for i in range(len(models)):
            for j in range(i + 1, len(models)):
                ...
                # Contingency table cells (only b and c are needed for McNemar's test)
                # a = ((pair_data[m1] == 0) & (pair_data[m2] == 0)).sum()  # Agree Safe
                # M1=0, M2=1 (Disagreement)
                b = ((pair_data[m1] == 0) & (pair_data[m2] == 1)).sum()
                # M1=1, M2=0 (Disagreement)
                c = ((pair_data[m1] == 1) & (pair_data[m2] == 0)).sum()
                # d = ((pair_data[m1] == 1) & (pair_data[m2] == 1)).sum()  # Agree Refusal
                ...
                if b_c == 0:
                    ...
                    chi2 = (
                        (abs(b - c) - 1) ** 2 / b_c if b_c >= 25 else (b - c) ** 2 / b_c
                    )  # Continuity correction if needed, but simplified here
