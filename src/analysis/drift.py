...
def calculate_drift_stats(df):
    ...
    for model in models:
        ...
        start_refusals = len(
            start_data[start_data["verdict"].isin(["REMOVED", "REFUSAL"])]
        )
        ...
        test_result = two_proportion_z_test(
            start_refusals, start_total, end_refusals, end_total
        )
...
def run_drift_analysis():
    ...
    print(
        f"✅ Drift Analysis Complete. Found {len(analysis_results)} models with history."
    )
    ...
    for r in analysis_results:
        ...
        print(
            f"  - {r['model']}: {r['start_refusal_rate']}% -> {r['end_refusal_rate']}% ({sig})"
        )
