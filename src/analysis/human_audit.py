...
def generate_sample(n=50):
    ...
    try:
        ...
        refusals = df[
            df["verdict"].isin(["REFUSAL", "REMOVED", "unsafe", "Hard Refusal"])
        ]
        allowed = df[
            ~df["verdict"].isin(["REFUSAL", "REMOVED", "unsafe", "Hard Refusal"])
        ]
        ...
        if len(refusals) < n_half or len(allowed) < n_half:
            print(
                f"⚠️ Warning: Not enough data for balanced split. Refusals: {len(refusals)}, Allowed: {len(allowed)}"
            )
        ...
        sample["AI_Label"] = sample["verdict"].apply(
            lambda x: 1 if x in ["REFUSAL", "REMOVED", "unsafe", "Hard Refusal"] else 0
        )
        ...
        if "prompt_text" not in sample.columns:
            if "text" in sample.columns:
                sample = sample.rename(columns={"text": "prompt_text"})
            elif "prompt" in sample.columns:
                sample = sample.rename(columns={"prompt": "prompt_text"})

        if "response_text" not in sample.columns:
            if "response" in sample.columns:
                sample = sample.rename(columns={"response": "response_text"})

        sample[export_cols].to_csv(OUTPUT_PATH, index=False)
        print(f"✅ Generated {len(sample)} samples to {OUTPUT_PATH}")
        print(
            "👉 ACTION REQUIRED: Open this CSV, fill 'Human_Label_EMPTY' with 0 (Safe) or 1 (Refusal), then run this script again."
        )
...
def calculate_agreement():
    ...
    try:
        ...
        if len(valid) < len(df):
            print(
                f"⚠️ Warning: Only {len(valid)}/{len(df)} rows labeled. Calculating on subset."
            )
        ...
        if kappa > 0.8:
            rating = "Almost Perfect Agreement 🟢"
        elif kappa > 0.6:
            rating = "Substantial Agreement 🟡"
        elif kappa > 0.4:
            rating = "Moderate Agreement 🟠"
        else:
            rating = "Poor Agreement 🔴"
