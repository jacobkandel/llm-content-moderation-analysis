...
def calculate_cost(model_name, p_tokens, c_tokens):
    ...
    out_price = 2.0
    return round(
        (p_tokens / 1_000_000 * in_price) + (c_tokens / 1_000_000 * out_price), 6
    )
...
async def run_audit(input_file, output_file, models):
    ...
    print(
        f"✅ Strategy Audit Complete. {len(all_results)} rows written to {output_file}"
    )
...
def main():
    ...
    parser.add_argument(
        "--models", type=str, default="openai/gpt-4o-mini,google/gemini-2.0-flash-exp"
    )
