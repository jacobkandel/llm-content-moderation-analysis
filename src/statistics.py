...
def calculate_pairwise_kappa(
    model1_verdicts: List[str], model2_verdicts: List[str]
) -> float:
    """
...
def calculate_power_analysis(
    effect_size: float, alpha: float = 0.05, power: float = 0.80, n_groups: int = 2
) -> int:
    """
...
def check_sample_adequacy(
    n_samples: int, n_categories: int, min_per_cell: int = 5
) -> Dict[str, any]:
    """
    ...
    return {
        ...
        "warning": (
            None
            if is_adequate
            else f"Need at least {min_recommended} samples for {n_categories} categories"
        ),
    }
