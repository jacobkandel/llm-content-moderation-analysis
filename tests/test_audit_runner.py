...
async def test_process_prompt_success():
    ...
    # 1. Mock API call
    with patch(
        "src.audit_runner.call_target_model_async", new_callable=AsyncMock
    ) as mock_api:
        ...
        with patch("src.audit_runner.Session"):
            ...
            with patch("src.audit_runner.check_cache", return_value=None):
                ...
                result = await audit_runner.process_prompt(
                    sem, prompt, model_name, force_rerun=True
                )
...
async def test_process_prompt_cache_hit():
    ...
    with patch("src.audit_runner.check_cache", return_value=mock_cached):
        sem = asyncio.Semaphore(1)
        result = await audit_runner.process_prompt(
            sem, prompt, model_name, force_rerun=False
        )
