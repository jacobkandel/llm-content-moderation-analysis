...
export const alt = 'Model Drift Analysis — Moderation Bias';
        ...
        (
            ...
            >
                ...
                <div style={{ color: '#ffffff', fontSize: 68, fontWeight: 900, lineHeight: 1.1, marginBottom: 24 }}>
                    Model Drift{' '}
                    <span style={{ color: '#cc0000' }}>Over Time</span>
                </div>

                {/* Subtitle */}
                <div style={{ color: '#999999', fontSize: 26, maxWidth: 700, lineHeight: 1.5 }}>
                    Track how LLM refusal rates shift week-over-week. Detect when models silently change their moderation policies.
                </div>

                {/* Stats row */}
                <div style={{ display: 'flex', gap: 40, marginTop: 48 }}>
                    {[
                        { label: 'Models Tracked', value: '20+' },
                        { label: 'Categories', value: '15' },
                        { label: 'Statistical Significance', value: 'p < 0.05' },
                    ].map(({ label, value }) => (
                        <div key={label} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            <div style={{ color: '#ffffff', fontSize: 32, fontWeight: 900 }}>{value}</div>
                ...
                <div style={{ position: 'absolute', bottom: 60, left: 80, color: '#555555', fontSize: 18 }}>
                    moderationbias.com/analysis/drift
                </div>
            </div>
        ),
        { ...size }
