...
export function StatCard({ title, value, description, change, icon, delay = 0, className = "" }: StatCardProps) {
    ...
    return (
        <motion.div
            ...
            className={`bg-card rounded-xl border border-border p-6 shadow-sm hover:shadow-md transition-shadow ${className}`}
        >
            ...
            {change !== undefined && (
                <div className="flex items-center gap-1 mt-2">
                    ...
                    {isNeutral && <Minus className="h-4 w-4 text-muted-foreground/70" />}
                    <span
