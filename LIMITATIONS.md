...
**Provenance caveat for the currently-published data:** those results were classified by `google/gemini-2.0-flash-001`, which was itself a subject at the time, so `gemini-2.0-flash` results specifically carry the self-preference confound. The held-out-judge configuration takes effect on the next full audit re-run, which will re-classify responses with an out-of-pool judge.

**Quantifying judge dependence.** Because all verdicts are machine-labeled (no human annotation is collected), judge dependence is bounded empirically with a *multi-judge robustness* check: `scripts/judge_robustness.py` re-judges a stratified sample with several alternative judge models and reports per-judge agreement, Fleiss' κ across judges, and the **verdict flip rate** (the share of items where the judges do not unanimously agree). Verdicts therefore reflect LLM-judge consensus, not human ground truth — a stated scope limitation of this benchmark. The harness is not part of the default pipeline because it makes live API calls; run it on demand:
